...
const DTCPanel: React.FC<DTCPanelProps> = ({ isConnected, onBack }) => {
  ...
  // Only auto-fetch if connected and user manually requests it
  const fetchAllDTCs = async () => {
    if (!isConnected) {
      toast.error('Device not connected. Please connect to an OBD2 device first.');
      return;
    }
    ...
    try {
      const [stored, pending, permanent] = await Promise.all([
        diagnosticService.getStoredDTCs().catch(() => []),
        diagnosticService.getPendingDTCs().catch(() => []),
        diagnosticService.getPermanentDTCs().catch(() => [])
      ]);
      ...
      if (totalCodes > 0) {
        toast.success(`Found ${totalCodes} diagnostic codes`);
      } else {
        toast.success('No diagnostic codes found - vehicle systems appear healthy');
      }
    } catch (error) {
      console.error('Failed to fetch DTCs:', error);
      toast.error('Failed to read diagnostic codes. Check your connection.');
    } finally {
      setIsLoading(false);
    }
  };
        ...
        <Card className="diagnostic-border">
          <CardContent className="pt-6">
            <div className="text-center text-muted-foreground">
              ...
              {isConnected && (
                <p className="text-sm mt-2">Click "Read DTCs" to scan for codes</p>
              )}
            </div>
          </CardContent>
        </Card>
  ...
  return (
    <div className="space-y-6">
      ...
      {!isConnected && (
        <Card className="diagnostic-border border-yellow-200 bg-yellow-50">
          <CardContent className="pt-6">
            <div className="text-center text-muted-foreground">
              ...
              <p className="text-sm mt-2">You can still navigate this interface in demo mode</p>
            </div>
          </CardContent>
        </Card>
      )}
