...
const VehicleSpecific = () => {
  ...
  const loadVehicleInfo = () => {
    try {
      ...
      // Case-insensitive search for make
      const makeData = VEHICLE_DATABASE.find(m => 
        m.id.toLowerCase() === make.toLowerCase() || 
      ...
      // Case-insensitive search for model
      const modelData = makeData.models?.find(m => 
      ...
      // Case-insensitive search for generation with year matching
      const generationData = modelData.generations?.find(g => 
        g.id.toLowerCase().includes(generation.toLowerCase()) || 
        g.yearRange.start.toString() === generation
      );
      ...
      // Case-insensitive search for engine with displacement matching
      const engineData = generationData.engines?.find(e => 
        e.id.toLowerCase().includes(engine.toLowerCase()) || 
        e.displacement.toLowerCase().includes(engine.toLowerCase())
      );
